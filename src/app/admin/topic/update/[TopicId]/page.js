...
import { PlusCircle, FileVideo, Image as ImageIcon, BookOpen } from "lucide-react";
...
const Update = () => {
    ...
    const getSubjects = async () => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API}/api/v1/subject/all`, { withCredentials: true });
    ...
    const getChapter = async (subject) => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API}/mdcat/chapter/subject/${subject}`, { withCredentials: true });
    ...
    const getTopic = async () => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API}/api/v1/topic/get/${TopicId}`, { withCredentials: true });
            const topic = res.data.topic;

            const subId = typeof topic.subject === "object" ? topic.subject._id : topic.subject;
            const chapId = typeof topic.chapter === "object" ? topic.chapter._id : topic.chapter;
            ...
            <div className="mx-auto max-w-4xl">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-zinc-900 flex items-center gap-3">
                        <PlusCircle className="text-zinc-600" /> Update Topic
                    </h1>
                    <p className="mt-2 text-zinc-500">Update the information below to modify this educational topic.</p>
                </div>
                <form onSubmit={updateTopic} className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
                    <div className="grid md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-semibold text-zinc-700 mb-2">Subject</label>
                            <select value={subjectId} onChange={(e) => { setSubjectId(e.target.value); setChapterId(""); getChapter(e.target.value); }} className="w-full rounded-xl border border-zinc-200 p-3 focus:ring-2 focus:ring-zinc-900 outline-none transition">
                    ...
                    <div>
                        ...
                        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-xl border border-zinc-200 p-3 outline-none focus:ring-2 focus:ring-zinc-900" />
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-zinc-700 mb-2">Description</label>
                        <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full rounded-xl border border-zinc-200 p-3 outline-none focus:ring-2 focus:ring-zinc-900" />
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-zinc-700 mb-2">Detailed Notes</label>
                        <textarea rows={6} value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full rounded-xl border border-zinc-200 p-3 outline-none focus:ring-2 focus:ring-zinc-900" />
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                        <div>
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-zinc-900 text-white font-bold py-4 rounded-xl hover:bg-zinc-800 transition shadow-lg flex items-center justify-center gap-2">
                        {loading ? "Updating..." : <><BookOpen size={20} /> Update Topic</>}
                    </button>
                </form>
            </div>
        </div>
